...
description: >-
  ...
  context assembly, scoped memory search, logical summarization triggers, and
...
- Retrieve memory by current chat scope first, not globally.
...
- Use scoped transcript search for exact old details when summaries/retrieval do not contain enough evidence.
...
2. Identify chat scope fields: platform, chat ID, thread/topic ID, user ID, session ID, lineage/root IDs.
...
5. Store summaries/chunks in a persistent memory index with scope metadata.
...
retrieved scoped summaries, ordered by relevance and recency
...
1. Search current chat/thread scope first.
...
7. When retrieved context is low-confidence, expand scoped search or ask for clarification instead of guessing.
...
- assembled prompt reports include token budget, tail size, retrieved chunk count, and retrieval scope;
...
- `session_search` may search globally instead of current Telegram chat/thread scope.
...
- User asks "what did we decide earlier?": relevant scoped summary is retrieved.
- User asks about exact old detail after compression: session search can recover it.
- User asks for an exact command/ID/error absent from summaries: agent searches scoped transcript instead of guessing.
