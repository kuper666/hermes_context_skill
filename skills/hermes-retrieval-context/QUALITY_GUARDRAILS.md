...
- Current chat/thread scope is the default boundary for retrieval.
...
2. If exact detail is needed and not present in retrieved chunks or recent tail, run scoped transcript/session search.
3. If scoped search fails, say that the exact prior detail was not found. Do not reconstruct it from memory.
4. Prefer "I found this in the scoped memory" over vague recall.
...
2. Retrieve more chunks from the same chat/thread scope.
...
Decision: Use scoped context retrieval instead of full Telegram session history because prompt tokens grow linearly.
Open task: Patch gateway prompt assembly so Telegram turns use ContextAssembler.
Critical details: Keep full transcript persisted; default retrieval scope is platform + chat_id + thread_id + user isolation.
...
- scope is broader than current chat/thread;
...
- If retrieval index is unavailable, fall back to recent tail plus scoped session search.
...
- The assembled prompt report shows token budget, tail size, retrieved chunk count, and retrieval scope.
